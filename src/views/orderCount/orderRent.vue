...
export default {
  ...
  data () {
    return {
      size: 'medium',
      loading: false,
      bredcrumb: [{id: 1, title: '首页'}, {id: 2, title: '租车管理'}, {id: 3, title: '租车数据统计'}],
      dataList: [],
      changeType: 1
    }
  },
  methods: {
    // 类型统计
    countType (callback) {
      this.$api.count.countType({typeId: 1}).then(res => {
        console.log('rent_countType', res.data)
    ...
    // 单量统计
    countTotal (callback) {
      this.$api.count.countTotal({typeId: 1}).then(res => {
        console.log('rent_countTotal', res.data)
    ...
    countExper (callback) {
      this.$api.count.countExper({typeId: 1}).then(res => {
        console.log('rent_countExper', res.data)
    ...
    formatData (data) {
      if (data) {
        ...
        let list = []
        data.forEach(item => {
          if (moment(item.endTime).isBefore('2017-01-01')) {
            beforeSeven.push(item)
          } else {
            let ll = {
              createTime: item.endTime,
    ...
    formatExperData (data) {
      let list = data.reduce((pre, current) => {
        let date = moment(current.endTime).year()
        pre[date] = pre[date] || []
        pre[date].push(current.count)
        return pre
      }, {})
      console.log('rent_format_exper', list)
