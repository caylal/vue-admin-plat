...
export default {
  ...
  data () {
    return {
      size: 'medium',
      loading: false,
      bredcrumb: [{id: 1, title: '首页'}, {id: 2, title: '接送机管理'}, {id: 3, title: '接送机数据统计'}],
      dataList: [],
      sevenGlData: [],
      eightGlData: [],
      eightJyData: [],
  ...
  methods: {
    // 类型统计
    countType (callback) {
      this.$api.count.countType({typeId: 2}).then(res => {
        console.log('airport_countType', res.data)
    ...
    // 单量统计
    countTotal (callback) {
      this.$api.count.countTotal({typeId: 2}).then(res => {
        console.log('airport_countTotal', res.data)
    ...
    countExper (callback) {
      this.$api.count.countExper({typeId: 2}).then(res => {
        console.log('airport_countExper', res.data)
