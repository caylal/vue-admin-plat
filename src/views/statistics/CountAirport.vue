<template>
  <div class="page-container">
    ...
    <el-table :data="pageResult.content"  max-height="480" stripe size="mini" style="width: 100%;" v-loading="loading" :element-loading-text="$t('action.loading')" :header-cell-style="{'background-color': 'rgb(217, 230, 249)'}">
      ...
      <el-table-column prop="aTotalCount" header-align="center" align="center" min-width="90" label="总次数"></el-table-column>
      <el-table-column prop="aTotalUsed" header-align="center" align="center" min-width="90" label="已使用次数"></el-table-column>
      <el-table-column header-align="center" align="center" min-width="90" label="剩余次数" >
        <template slot-scope="scope">
          {{scope.row.aTotalCount - scope.row.aTotalUsed}}
        </template>
      </el-table-column>
...
export default {
  ...
  data () {
    return {
      ...
      bredcrumb: [{id: 1, title: '首页'}, {id: 2, title: '接送机管理'}, {id: 3, title: '接送机使用次数统计'}],
  ...
  methods: {
    ...
    findPage (data) {
      ...
      this.$api.count.countAirport(this.pageRequest).then(res => {
