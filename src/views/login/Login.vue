...
export default {
    ...
    login () {
      ...
      this.$api.user.login(userInfo).then(res => {
        if (res.code !== 200) {
          ...
          sessionStorage.setItem('userInfo', userInfo.account) // 保存用户到本地会话
  ...
  created () {
    let {agentName, recordNo, callTime} = this.$route.query
    console.log(agentName, recordNo, callTime)
    console.log('query:', this.$route.query)
    if (agentName && recordNo && callTime) {
      let data = Object.assign({}, { agentName: agentName, recordNo: recordNo, callTime: callTime })
      this.$store.commit('setAgentInfo', data)
      this.login()
    }
  }
}
